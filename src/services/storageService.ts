      ...
      try {
        ...
        if (snap.empty) {
          ...
          for (const drive of DEFAULT_DRIVES) {
            ...
            seeded.push({
              ...
              remainingStorage: drive.remainingStorage
            });
          }
        ...
        const configs = snap.docs.map(d => {
          ...
          if (data.connectedStatus !== 'connected') {
            const defDrive = DEFAULT_DRIVES.find(dd => dd.id === data.id);
            return {
              ...data,
              totalStorage: defDrive ? defDrive.totalStorage : 0,
              usedStorage: defDrive ? defDrive.usedStorage : 0,
              remainingStorage: defDrive ? defDrive.remainingStorage : 0,
            };
          }
          return data;
        });
